...
class Booking extends CI_Controller {
    ...
    public function index() {
        ...
        $dtb = $this->ModelBooking->getDataWhere('temp', ['email_user' => $this->session->userdata('email')])->num_rows();
    ...
    // function untuk menambah bookingan
    public function tambahBooking() {
        ...
        $isi = [
            'id_buku' => $id_buku, 'judul_buku' => $d->judul_buku,
            'id_user' => $this->session->userdata('id_user'), 
        ...
        $temp = $this->ModelBooking->getDataWhere('temp', ['id_user' => $userid, 'id_buku' => $id_buku])->num_rows();
        ...
        //jika buku yang di klik sudah di kranjang
        if ($temp > 0) {
    ...
    public function hapusbooking() {
        $id_buku = $this->uri->segment(3);
        $id_user = $this->session->userdata('id_user');
        $before_delete = $this->db->query("select * from temp where id_user='$id_user'")->num_rows();
        $this->ModelBooking->deleteData(['id_buku' => $id_buku], 'temp');
        $current_booking = $this->db->query("select * from temp where id_user='$id_user'")->num_rows();

        if ($current_booking < 1 && $before_delete > $current_booking) {
            // $this->session->set_flashdata('pesan', '<div class="alert alert-massege alert-danger" role="alert">Tidak Ada Buku dikeranjang</div>');
            $this->session->set_flashdata("pesan_sweetalert", "<script> Swal.fire('Berhasil', 'Buku sudah dihapus dari kranjang, Tidak ada buku di kranjang sekarang', 'success') </script>");
            redirect(base_url());
        } else if ($before_delete > $current_booking) {
            $this->session->set_flashdata("pesan_sweetalert", "<script> Swal.fire('Berhasil', 'Buku sudah dihapus dari kranjang', 'success') </script>");
    ...
    public function info() {
        ...
        $data['user'] = $this->ModelUser->cekData(['id' => $this->session->userdata('id_user')])->row_array();
